...
* `GET /api/membres/{id}/carte` & `GET /api/membres/{id}/fiche` : Génère la fiche ou carte de membre officielle (HTML responsive prêt pour impression ou JSON) avec l'image du QR Code encodée en base64 inline, lien de scan direct (`/membres/scan/{token}`) et le taux de participation annuel.
